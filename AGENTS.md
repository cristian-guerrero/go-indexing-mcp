...
- **Async periodic save**: `SaveSnapshot()` copies records under the lock (~3ms for 10k records), releases the lock, then gob-encodes and writes disk WITHOUT holding any lock. The main indexing goroutine never blocks on serialization
- **Atomic save guard**: `saveInFlight.CompareAndSwap` ensures only one async save runs at a time; concurrent requests are safely skipped (data is in memory, captured by next save or final save)
- **Windows rename retry**: `SaveSnapshot` retries rename (3 attempts, 10/20ms backoff) to handle transient file handle locks after `Close()` on Windows
- **Trigram skip in periodic saves**: `saveLocked(skipTrigrams bool)` skips trigram persistence mid-index. Trigrams are rebuilt only on final save, since `grep_code` never runs during active indexing
